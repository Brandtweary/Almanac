...
import {RecallPool,PROVISIONAL_RECALL_POLICY} from "../src/kg/recall-pool.ts";
import {formatTranscript} from "../src/pipeline.ts";
...
export interface CandidateProfile {temperature?:number;topP?:number;id:string;name?:string;contextWindow:number;maxOutputTokens:number;maxStageOutputTokens?:number;providerRouting?:{only?:string[];order?:string[]};reasoning:boolean;thinkingLevel?:ThinkingLevel;prices?:{input:number;output:number;cacheRead:number;cacheWrite:number};}
...
export async function runStockCase(fixture:StockCase,profile:CandidateProfile,apiKey:string,options:{maxCompletions:number;timeoutMs:number;spendAvailable:()=>boolean;shellHelperDirectory?:string;creativePrompt?:{text:string;sourceDigest:string};stream?:StreamFn}){
 if(options.creativePrompt&&(fixture.track!=="creative"||(fixture.roleCase?.role??"chat")!=="chat"||!options.creativePrompt.text.trim()||!/^([a-f0-9]{64})$/.test(options.creativePrompt.sourceDigest)))throw new Error("Frozen prompt overrides are scoped to the creative comparison track");
 ...
 const graph=Graph.empty();for(const term of fixture.roleCase?.initialTerms??[]){graph.getOrCreate(term.label,term.description);for(const alias of term.aliases??[])graph.addAlias(term.label,alias);}
 const initialState=structuredClone(graph.serialize());const seed=fixture.backgroundSeed;const stt=structuredClone(seed?.stt??emptySttLexicon());const actions:string[]=[];const flags:any[]=[];
 const matchingState=()=>Object.fromEntries((seed?.matchingProbes??[]).map(text=>[text,graph.termMatch(text).map(term=>term.label).sort()]));
 ...
 if(seed?.matchingProbes)receipt.matchingBefore=matchingState();
 const maintenance=emptyMaintenance();const fixtureEvents:any[]=[];let peerEdited=false;
 const role=fixture.roleCase?.role??"chat";if(role==="memory")prepareMaintenance(graph,maintenance);let windowOutcome:StageOutcome|undefined;let workingSummary=seed?.summary??"";const findings:any[]=[];const stageUsage={input:0,output:0};
 ...
 if(seed?.retrieval){
  const pool=new RecallPool(PROVISIONAL_RECALL_POLICY);pool.observe("message",seed.retrieval.messageText,graph);
  // Synthetic prior chat admission uses a conservative byte count; this is not model tokenizer evidence.
  const prepared=await pool.prepare(graph,async block=>Buffer.byteLength(block??""),65536);
  const delivery=pool.commitDelivery(prepared,"synthetic-prior-chat");
  if(objectDigest(delivery.terms.map(t=>t.label).sort())!==objectDigest([...seed.retrieval.expectedLabels].sort()))throw new Error("Synthetic retrieval differs from frozen expected labels");
  const message={role:"memory-delivery",receipt:delivery,timestamp:1};
  transcriptRecords.push({id:"retrieval-0",title:"Admitted prior chat retrieval",provenance:"retrieved personal memory, not a new user statement",text:formatTranscript([message as any])});
  receipt.syntheticRetrieval={delivery,measurement:"conservative UTF-8 bytes for synthetic prior chat admission; not native token measurement"};
 }
 ...
 const inspector=createMemoryInspector({assertActive,records:collection=>{
  ...
  if(collection==="actions")return [{id:"committed",title:"Prior actions",provenance:"committed action history",text:seed?.priorAudit?.map(entry=>JSON.stringify(entry)).join("\n")||"(no prior actions)"},{id:"draft",title:"Draft actions",provenance:"uncommitted actions",text:actions.join("\n")}];
  if(collection==="voice")return seed?.voice?[{id:"raw",title:"Raw and corrected voice evidence",provenance:`raw speech recognition evidence, utterance ${seed.voice.utteranceId}`,text:JSON.stringify(seed.voice)}]:[];
 ...
 if(fixture.shellCase){
  ...
  const mutationTools=role==="summary"?[]:createPipelineTools({getGraph:()=>graph,getSttLexicon:()=>stt,embed:async()=>null,assertActive,maintenance,voiceEvidence:seed?.voice,phonemize:seed?.pronunciation?async texts=>{const fixture=seed.pronunciation!;if(texts.some(text=>!fixture.entries[text]))throw new Error(`No frozen pronunciation for synthetic spans: ${texts.filter(text=>!fixture.entries[text]).join(", ")}`);return{engine:fixture.engine,phonemes:texts.map(text=>fixture.entries[text])};}:undefined,record:line=>actions.push(line),addFlag:flag=>flags.push(flag)});
  ...
  const auditHandoff=createAuditHandoffTool({put:(finding,quotes)=>{assertActive();const evidence=quotes.map(ref=>{const record=transcriptRecords.find(r=>r.id===ref.recordId);if(!record||record.provenance!=="user statement"||!ref.quote.trim()||!record.text.includes(ref.quote))throw new Error("Handoff quote must match admitted user evidence");return {...ref,role:"user"};});if(finding.kind==="spelling"){if(!seed?.voice||finding.utteranceId!==seed.voice.utteranceId||!stt.mistranscriptions.some(row=>row.utteranceId===finding.utteranceId&&row.transcribed===finding.transcribed&&row.spoken===finding.spoken&&row.status!=="rejected"))throw new Error("Spelling handoff requires accepted current-utterance speech evidence");}else if(finding.kind!=="stale-description"||!graph.thoughts.has(finding.termId)||!finding.reason?.trim())throw new Error("Stale-description handoff requires an existing term and reason");findings.push({...finding,evidence});}});
 ...
 if(options.creativePrompt){
  systemPrompt=options.creativePrompt.text;receipt.creativePrompt={sourceDigest:options.creativePrompt.sourceDigest,scope:"frozen production builder output"};
 }
 ...
 const stream:StreamFn=(selected,context,settings)=>{
  ...
  return streamSimple(selected,context,{...settings,apiKey,temperature:profile.temperature??0,maxTokens:Math.min(profile.maxOutputTokens,remaining),reasoning:profile.thinkingLevel,maxRetries:0,timeoutMs:options.timeoutMs,
   onPayload(payload){const controlled=hostedPayload(payload as object,profile);requests.push(controlled);return controlled;},
  });
 };
...
const meta:any={httpStatus:response.status};providerReceipts.push(meta);
 ...
 try{
  await libraries.run({library,observe},async()=>{
   for(const step of fixture.steps){
    ...
    if(role!=="chat"&&role!=="compaction"){
     const ctx={bufferBlock:"Read memory_inspect(actions,id=committed) for prior actions; draft actions are actions/draft.",isVoiceTurn:!!seed?.voice};
    ...
    const assistants=generated.filter(m=>m.role==="assistant") as any[];
    turnOutcomes.push({turn:turnOutcomes.length+1,stopReason:assistants.at(-1)?.stopReason??"missing",intermediateFailures:assistants.filter(m=>!["stop","toolUse"].includes(m.stopReason)).map(m=>m.stopReason)});
   }
  });
  ...
  checks.push({id:"valid_completion",passed:turnOutcomes.length===fixture.steps.length&&turnOutcomes.every(t=>t.stopReason==="stop"&&!t.intermediateFailures.length)&&last?.stopReason==="stop"&&Boolean(receipt.answer.trim()||(stageRole&&["no-op","completed"].includes(windowOutcome?.kind??""))),critical:true,evidence:JSON.stringify({lastStopReason:last?.stopReason??"missing",turnOutcomes})});
  ...
  if(role==="chat"&&!fixture.shellCase&&(fixture.track!=="creative"||fixture.assertions.some(a=>a.kind==="citation_identity"))){
 ...
 if(seed?.matchingProbes)receipt.matchingAfter=matchingState();
 receipt.auditFindings=findings;receipt.maintenance=maintenance;receipt.fixtureEvents=fixtureEvents;receipt.finalState=graph.serialize();receipt.stt=stt;receipt.actions=actions;receipt.flags=flags;receipt.toolCalls=calls;receipt.libraryRequests=library.requests;
