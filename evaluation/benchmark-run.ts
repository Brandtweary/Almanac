...
for(const p of profiles)validateHostedIdentity(p);
if(!values["prior-budget"])throw new Error("Explicit cumulative prior budget required");
const prior=JSON.parse(readFileSync(values["prior-budget"],"utf8"));
if(prior.ceilingUSD!==maxSpend)throw new Error("Prior budget ceiling must equal max-spend");
if(values.browser){
...
snapshot("src");snapshot("evaluation");for(const name of ["package.json","package-lock.json"]){const bytes=readFileSync(path.join(root,name));files[name]=createHash("sha256").update(bytes).digest("hex");if(!values.resume)writeFileSync(path.join(output,"snapshot",name),bytes);}
...
const ledgerFile=path.join(output,"budget-ledger.json");if(values.resume&&!existsSync(ledgerFile))throw new Error("Resume requires existing budget ledger");
const budget=new CampaignBudget(ledgerFile,prior);
let spent=budget.totals().reportedUSD,reserved=budget.totals().reservedUSD;const rows:any[]=[];let next=0;
const queue=selected.flatMap(fixture=>profiles.map(profile=>({profile,fixture})));
function report(){
 spent=budget.totals().reportedUSD;reserved=budget.totals().reservedUSD;
...
async function worker(){while(next<queue.length){const {profile,fixture}=queue[next++];const filename=profile.id.replaceAll("/","__")+"--"+fixture.id+".json";const target=path.join(output,filename);
 if(existsSync(target)){const old=JSON.parse(readFileSync(target,"utf8"));if(old.caseDigest!==objectDigest(fixture)||objectDigest(old.profile)!==objectDigest(profile))throw new Error("Receipt identity mismatch");if(!budget.entries[filename])throw new Error("Receipt has no durable reservation");if(budget.entries[filename].status==="reserved")budget.settle(filename,old.metrics.costUSD,old.usageIncomplete);rows.push(old);continue;}
 if(budget.entries[filename]){rows.push({caseId:fixture.id,profile,status:"interrupted_unmeasured",metrics:{seconds:0,costUSD:0},checks:[]});report();continue;}
 const exposure=maxCompletions*(65536*Math.max(profile.prices!.input,profile.prices!.cacheRead,profile.prices!.cacheWrite)+profile.maxOutputTokens*profile.prices!.output)/1e6;
 if(!budget.reserve(filename,exposure)){rows.push({caseId:fixture.id,profile,status:"budget_not_run",metrics:{seconds:0,costUSD:0},checks:[]});report();continue;}
 ...
 result.exposureReserveUSD=exposure;result.rubric=fixture.rubric;result.artifacts=[filename];
 const serialized=JSON.stringify(result,null,2).split(apiKey).join("[redacted credential]");writeFileSync(target,serialized+"\n");budget.settle(filename,result.metrics.costUSD,result.usageIncomplete);rows.push(result);report();
 }}
await Promise.all([worker(),worker()]);report();releaseOwnership();
