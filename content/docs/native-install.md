...
For Wikipedia use its exact acquired nopic SHA-256, `--pack-id wikipedia-en-nopic-2026-06`, `--source-base-url https://en.wikipedia.org/wiki`, `--license CC-BY-SA-4.0`, `--selection-policy canonical-html` and the matching checked inspection receipt. Canonical HTML includes full articles; redirects resolve to their canonical targets, and non-HTML assets are not vector documents. The `ifixit-repair-v1` policy decides from the entry path, admitting the guide, device, teardown and reference sections and refusing member profiles and site navigation. The `gutenberg-books-v1` policy admits each book's own HTML and refuses the pages the Gutenberg scraper generates: a book's cover page by its path, and its author and catalog pages by the site header they carry. The Appropedia policy requires an English article-body language declaration, applies the documented CC-BY-SA-4.0 default when page metadata selects that default, and respects explicit alternative licenses plus the inspection receipt’s source-bound exception map. Copyright discussion, bibliographic references and permission acknowledgements are not blanket exclusions; the four recorded unresolved contrary notices are source-serving choices, not legal verdicts, and this selected text scope does not clear every archive image or imported document. The `wikisource-mainspace-v1` policy decides from the entry path alone, admitting the assembled works and refusing the `Page:` and `Index:` proofreading namespaces, which hold one entry per scanned page of a work already indexed under its own title; it resolves no license of its own, so the archive's `--license` declaration stands and each page's own licensing template governs reuse.

Beside a library already being served, a preparation joins the active union only once its own dense index is complete, because the union is qualified only while every member's index is; the first archive of an empty library joins at once, so its full-text search and reading are available while it indexes. `--no-activate` holds a completed generation out of the union, which is how its article spans are precomputed before it serves; running the command again without it joins.
...
A crawl whose books are PDF entries holds text its own full-text index never reads. `tools/build_survivor_text_zim.py` builds a text archive from it, which is prepared like any other archive; the crawl itself is published among the originals so that a book's source is its scan. Publish it with `oracle_content.ingest.publish_original(store, crawl, sha256, True)`, which hardlinks on the same filesystem.
