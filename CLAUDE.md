...
- `src/corpus-tools.ts` exposes search/read, a listing of the installed collections grouped by the category each was prepared under, and immutable evidence handles; `oracle-context.ts` preserves evidence identity through bounded context and compaction.
- `src/answer-sources.ts` also builds the per-answer clipboard payload — the answer with its citation handles resolved to source links, followed by the footer's own sources — and `src/answer-speech.ts` admits one on-demand reading at a time through a port `main.ts` installs over the shared synthesizer, so a reading never supersedes the agent's own playback; `MessageList` renders both controls below the sources footer.
...
- `content/` contains the Python corpus service, staged ingestion, immutable SQLite catalog, native ZIM lexical integration and persistent dense-index adapters. A native generation's article blocks, segmentation spans and semantic representatives are precomputed once beside it rather than rebuilt per search; the artifact never defines a passage, so absent or partial coverage falls back to segmenting at query time. Generations index disjoint archives, so cross-generation ranking rests on dense similarity — comparable across collections under the one pinned encoder — and never on a passage handle; a search envelope also reports the ranked set its page slices, because no stage code describes a relevant collection whose hits begin below the page. `content/oracle_content/integrity/` guards the installed archives' bytes against a committed Merkle manifest (`deploy/integrity/`): an offline O_DIRECT scrub, per-document quarantine as a read-time overlay, and journalled in-place mending from parity or upstream; `content/docs/integrity.md` states its contract.
- `deploy/` contains the single setup entry, acquisition manifests and offline packaging; `docs/install.md` is the installation contract. A pack manifest itemizes the installed footprint its disk reservations stand for, covering indexing and not only download, and setup refuses a component whose declared bytes its own stated derivation does not reproduce. Each component also names the filesystem holding it, so setup charges the vector store, the content state and the data root once each rather than demanding the same bytes of one disk twice.
