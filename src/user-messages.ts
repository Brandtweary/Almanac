import { validAttachments, validImageBlocks } from "./attachment-limits.js";
...
export function validUserMessage(message: AgentMessage): boolean {
  ...
  if (Array.isArray(message.content) && !message.content.every(block => block &&
    (block.type === "text" ? typeof block.text === "string" : block.type === "image"))) return false;
  if (Array.isArray(message.content) && !validImageBlocks(message.content)) return false;
