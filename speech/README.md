...
Each connection receives `Ready`, accumulates bounded `Text` frames, and begins generation at `Eos`; the browser already supplies sentence-sized sessions. Audio streams as 24 kHz mono float PCM `Audio` frames, followed by a `Text` timing receipt and normal close. A failure closes abnormally. One generation owns the engine at a time; busy clients receive 1013. Synthesis runs in a persistent spawned process. An abandoned session — barge-in, mute, a new chat — cancels generation cooperatively: the worker stops at its next audio frame, acknowledges, and stays loaded for the next sentence. Native work always finishes before admission is released, and a worker that does not acknowledge within the reset budget, one that failed, or one interrupted while still loading is killed and reaped as before, with the next request loading a fresh engine. Queued audio, frame size, text length and session lifetime are bounded.
