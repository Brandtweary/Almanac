...
Startup requires all assets locally and disables model-hub network access. `GET /health` reports readiness after model/voice loading. The only synthesis path is `/api/tts_streaming?voice=alba&format=PcmMessagePack`. `cfg_alpha` is accepted for compatibility but does not change Pocket's sampling configuration.

Each connection receives `Ready`, accumulates bounded `Text` frames, and begins generation at `Eos`; the browser already supplies sentence-sized sessions. Audio streams as 24 kHz mono float PCM `Audio` frames, followed by a `Text` timing receipt and normal close. A failure closes abnormally. One generation owns the engine at a time; busy clients receive 1013. Disconnect cancels at a model chunk boundary, and admission remains occupied until that worker exits. Queued audio, frame size, text length and session lifetime are bounded.
