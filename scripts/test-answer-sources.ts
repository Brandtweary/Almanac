...
try{
 ...
 const copyButton=page.locator('answer-controls').getByRole('button',{name:'Copy',exact:true});assert.equal(await copyButton.count(),2);assert.equal(await page.locator('answer-controls').getByRole('button',{name:'Read aloud',exact:true}).count(),0);
