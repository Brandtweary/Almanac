...
Verification does not end at install. Once an archive is prepared, admit it to [corpus integrity](../content/docs/integrity.md) with `content/tools/integrity.py`: one rate-limited read records its 4 MiB leaf hashes, cross-checked against the upstream metalink pieces, into the committed manifest under `deploy/integrity/`, with local parity and a map of which documents each byte range holds. A scheduled scrub then re-reads every leaf from the disk, damaged documents are refused and named while everything else keeps serving, and damaged leaves are repaired in place from parity or the upstream mirrors with bytes that verify against the manifest. Admit an upstream archive while its edition is still listed upstream, since the mirrors are its network repair source.
